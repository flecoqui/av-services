...
# stream a mkv file to rtmp server to emulate live feed with encoding:
...
# stream a mkv file to rtmp server to emulate live feed without encoding:
ffmpeg.exe -v verbose  -re -stream_loop -1 -i camera-300s.mkv -codec copy -bsf:v h264_mp4toannexb   -f flv rtmp://testrtmprtspvm.eastus2.cloudapp.azure.com:1935/live/stream
