...
if [[ "${action}" == "test" ]] ; then
    ...
    if [[ -z ${sinkContainerId} ]] ; then
        ...
        docker build  --build-arg  AV_PORT_RTMP=${AV_RTMP_PORT} --build-arg  AV_PORT_SSL=8443 --build-arg  AV_PORT_HTTP=80 --build-arg  AV_PORT_HLS=8080  --build-arg  AV_HOSTNAME=localhost --build-arg  AV_COMPANYNAME=contoso -t ${AV_IMAGE_FOLDER}/av-rtmp-sink-${AV_FLAVOR} ../../av-rtmp-sink/alpine 
