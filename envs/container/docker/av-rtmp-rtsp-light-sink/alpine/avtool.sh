...
AV_SERVICE=av-rtmp-rtsp-light-sink
AV_FLAVOR=alpine
...
if [[ "${action}" == "deploy" ]] ; then
    ...
    docker build  --build-arg  AV_PORT_RTSP=${AV_PORT_RTSP} --build-arg  AV_PORT_RTMP=${AV_PORT_RTMP} --build-arg  AV_HOSTNAME=${AV_HOSTNAME} -t ${AV_IMAGE_FOLDER}/${AV_IMAGE_NAME} . 
    checkError
    docker run  -d -it -p ${AV_PORT_RTMP}:${AV_PORT_RTMP}/tcp -p ${AV_PORT_RTSP}:${AV_PORT_RTSP}/tcp  -e PORT_RTSP=${AV_PORT_RTSP} -e PORT_RTMP=${AV_PORT_RTMP} -e HOSTNAME=${AV_HOSTNAME}  --name ${AV_CONTAINER_NAME} ${AV_IMAGE_FOLDER}/${AV_IMAGE_NAME} 
    checkError
